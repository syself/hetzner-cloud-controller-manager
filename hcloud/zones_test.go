...
func TestGetZone(t *testing.T) {
	...
	zones := newZones(env.Client, env.RobotClient, "node6")
	...
	zones = newZones(env.Client, env.RobotClient, "bm-server1")
	zone, err = zones.GetZone(context.TODO())
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
...
func TestGetZoneForServer(t *testing.T) {
	...
	defer env.Teardown()
	env.Mux.HandleFunc("/servers", func(w http.ResponseWriter, r *http.Request) {
		if r.URL.RawQuery != "name=hcloud-node15" || strings.HasPrefix(r.URL.Path, "/robot") {
	...
	zone, err = zones.GetZoneByNodeName(context.TODO(), "bm-server1")
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	if zone.Region != "eu-central" {
